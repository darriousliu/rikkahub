...
| M38 | 聊天消息翻译、折叠、清除与重启 | JVM | 需真实模型与桌面 GUI | 翻译预存 Hello；折叠/展开；退出重启；清除后再次重启 | 译文正常展示和恢复，清除后不重现 | 历史 Pass（修复原生包后） | 审查后清除恢复原内存行为，见[说明](cmp-session-migration-audit.md)；gpt-5.6-terra 与主 agent 操作电脑插件；1 次 DeepSeek 请求；截图、AX 与 SQLite 独立检查通过；[回归记录](cmp-gui-regression.md#2026-09-09聊天消息翻译) | 2026-09-09 |
| M39 | 会话压缩、保留最近消息与重启 | JVM | 需真实模型与桌面 GUI | 先验证保留 32 条不足，再设目标 500/保留 1，退出重启 | 不足时不改数据；成功时摘要与最近消息落盘，旧建议清空 | Pass | Terra 与主 agent 操作电脑插件；1 次 DeepSeek 请求；[回归记录](cmp-gui-regression.md#2026-09-09会话上下文压缩) | 2026-09-09 |
