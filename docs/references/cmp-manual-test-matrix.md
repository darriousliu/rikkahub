...
| M38 | 聊天消息翻译、折叠、清除与重启 | JVM | 需真实模型与桌面 GUI | 翻译预存 Hello；折叠/展开；退出重启；清除后再次重启 | 译文正常展示和恢复，清除后不重现 | Pass（修复原生包后） | gpt-5.6-terra 与主 agent 操作电脑插件；1 次 DeepSeek 请求；截图、AX 与 SQLite 独立检查通过；[回归记录](cmp-gui-regression.md#2026-09-09聊天消息翻译) | 2026-09-09 |
