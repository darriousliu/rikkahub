...
internal actual val screenTimeAccess: ScreenTimeAccess = object : ScreenTimeAccess {
    override suspend fun checkPermission() {
        if (!FileKit.context.hasUsageStatsPermission()) {
            KoinPlatform.getKoin().get<AppEventBus>().emit(AppEvent.OpenUsageAccessSettings)
            throw LocalToolAccessException("NO_PERMISSION",
    ...
    override suspend fun requestPermission(): Boolean = FileKit.context.hasUsageStatsPermission()
    override suspend fun query(startMs: Long, endMs: Long): List<AppUsage> {
        val context = FileKit.context
        val pm = context.packageManager
        val manager = context.getSystemService(Context.USAGE_STATS_SERVICE) as UsageStatsManager
        return computeForegroundTime(manager, startMs, endMs, resolveLauncherPackages(pm)).map { (id, millis) ->
            AppUsage(id, resolveAppName(pm, id), millis)
        }
    }
}
