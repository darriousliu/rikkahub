...
internal class SseClientTransport(
    ...
    override suspend fun start() {
        ...
        client.sse(
            request = {
                ...
                headers {
                    ...
                    append("User-Agent", "RikkaHub/${BuildConfig.VERSION_NAME}")
                }
            }
        ) {
            try {
                ...
                incoming.collect { event ->
                    ...
                    when (type) {
                        ...
                        "endpoint" -> {
                            ...
                            Logger.i(TAG) { "onEvent: endpoint: $endpointData" }
        ...
        withTimeout(30000) {
            endpoint.await()
            Logger.i(TAG) { "start: Connected to endpoint ${endpoint.getCompleted()}" }
        }
    }
    ...
    override suspend fun send(message: JSONRPCMessage) {
        ...
        Logger.i(TAG) { "send: POSTing to endpoint ${endpoint.getCompleted()} - $message" }
        ...
        try {
            ...
            if (!response.status.isSuccess()) {
                ...
                Logger.i(TAG) { "send: POST to endpoint ${endpoint.getCompleted()} successful" }
            }
        } catch (e: Exception) {
            _onError(e)
            throw e
