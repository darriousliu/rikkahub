...
const val DEFAULT_THEME_ID = "sakura"
...
fun Settings.findModelById(uuid: Uuid?, fallback: Uuid? = null): Model? {
    ...
    return uuid?.let { providers.findModelById(it) }
        ?: fallback?.let { providers.findModelById(it) }
}
...
fun Settings.getSelectedTTSProvider(): TTSProviderSetting? {
    return ttsProviders.find { it.id == selectedTTSProviderId }
...
val DEFAULT_TTS_PROVIDERS = listOfNotNull(
    if (!currentPlatformKind.isLinux)
