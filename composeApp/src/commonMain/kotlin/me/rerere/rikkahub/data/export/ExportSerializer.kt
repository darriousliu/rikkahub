...
import me.rerere.common.utils.readContentFromUri
...
interface ExportSerializer<T> {
    ...
    fun import(context: PlatformContext, uri: Uri): Result<T>
    ...
    // 读取 URI 内容的便捷方法
    fun readUri(context: PlatformContext, uri: Uri): String {
        return readContentFromUri(context,uri).decodeToString()
    }

    fun getUriFileName(context: PlatformContext, uri: Uri): String? {
...
object ModeInjectionSerializer : ExportSerializer<PromptInjection.ModeInjection> {
    ...
    override fun import(context: PlatformContext, uri: Uri): Result<PromptInjection.ModeInjection> {
        return runCatching {
...
object LorebookSerializer : ExportSerializer<Lorebook> {
    ...
    override fun import(context: PlatformContext, uri: Uri): Result<Lorebook> {
        return runCatching {
            val json = readUri(context, uri)
            // 首先尝试解析为自己的格式
            tryImportNative(json)
    ...
    private fun tryImportSillyTavern(json: String, fileName: String?): Lorebook? {
        return runCatching {
            ...
            Lorebook(
                id = Uuid.random(),
                name = fileName ?: Clock.System.now().toLocalDateTime(TimeZone.currentSystemDefault()).toLocalString(),
                description = "",
                enabled = true,
