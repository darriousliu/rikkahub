...
private fun UnifiedLogList(logs: List<LogEntry>, modifier: Modifier = Modifier) {
    ...
    LazyColumn(
        ...
        items(sortedLogs, key = { it.id }, contentType = { it::class.simpleName }) { log ->
            when (log) {
    ...
    Card(
        ...
        Column(
            ...
            Row(
                ...
                Text(
                    text = Instant.fromEpochMilliseconds(log.timestamp).toLocalDateTime(TimeZone.currentSystemDefault())
                        .formatLocalizedTime("HH:mm:ss"),
...
private fun RequestLogDetail(log: LogEntry.RequestLog) {
    LazyColumn(
        ...
        item {
            DetailSection(
                "Time",
                Instant.fromEpochMilliseconds(log.timestamp).toLocalDateTime(TimeZone.currentSystemDefault())
                    .formatLocalizedTime("yyyy-MM-dd HH:mm:ss.SSS")
            )
        }
