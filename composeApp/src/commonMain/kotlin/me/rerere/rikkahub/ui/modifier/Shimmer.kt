...
fun Modifier.shimmer(
    ...
    if (!isLoading) {
        ...
        // 将角度转换为弧度
        val angleRad = angle.toDouble().toRadians().toFloat()
        // 计算渐变颜色的列表
        ...
        this
            ...
            .drawWithContent { // 使用 drawWithContent 获取绘制上下文
                ...
                // 计算渐变的起始点和结束点，考虑角度
                val startX = currentOffset * cos(angleRad)
                val startY = currentOffset * sin(angleRad)
                val endX = (currentOffset + gradientWidth) * cos(angleRad)
                val endY = (currentOffset + gradientWidth) * sin(angleRad)
                // 创建线性渐变 Brush
...
private inline fun Double.toRadians(): Double = this * (PI / 180.0)
