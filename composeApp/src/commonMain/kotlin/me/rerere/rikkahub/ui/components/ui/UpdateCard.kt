...
import me.rerere.rikkahub.utils.downloadUpdate
...
fun UpdateCard(vm: ChatVM, buildInfo: PlatformBuildInfo) {
    ...
    state.onSuccess { info ->
        ...
        if (showDetail) {
            val downloadingMessage = stringResource(Res.string.update_card_downloading)
            ...
            ModalBottomSheet(
                ...
                Column(
                    ...
                    Text(
                        text = Instant.parse(info.publishedAt).toLocalizedDateTime(),
                        style = MaterialTheme.typography.bodySmall,
                        color = MaterialTheme.colorScheme.primary
                    )
                    MarkdownBlock(
                    ...
                    if (currentPlatformKind == PlatformKind.ANDROID) {
                        ...
                        OutlinedCard(
                            onClick = {
                                externalUriOpener.open("https://github.com/darriousliu/rikkahub/releases")
                                showDetail = false
                            },
                        ) {
                            ListItem(
                                ...
                                supportingContent = {
                                    Text(
                                        if (currentPlatformKind == PlatformKind.IOS) {
                                            "Download IPA and sign it to install"
                                        } else {
